...
import { getData, insertUser, deleteUser, updateUser  } from '@/actions/home'
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text,record) => (
        <input 
          defaultValue={text} 
          disabled={parseInt(record.count) === 0 ? false : true} 
    ...
    },
    {
      ...
      render: text => (
        <Space size="middle">
          <Button onClick={() => editBefore(text)} disabled={parseInt(text.count) === 0 ? false : true}>编辑</Button>
          <Button onClick={() => deleteUserFn(text)} disabled={parseInt(text.count) === 0 ? false : true}>删除</Button>
        </Space>
      ),
    }
  ]
